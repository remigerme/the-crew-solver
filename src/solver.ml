...
let rec play s =
  ...
  if game_over s then ()
    ...
    let playable_hand =
      if Dynarray.length s.current_trick.cards == 0 then p.hand
      else
        let first_card = Dynarray.get s.current_trick.cards 0 in
    ...
    for i = 0 to Dynarray.length playable_hand - 1 do
      ...
      let old_trick =
        { s.current_trick with cards = Dynarray.copy s.current_trick.cards }
      in
      let old_players =
        Array.map
      ...
      (* Updating state *)
      s.players.(pid) <- new_p;
      Dynarray.add_last s.current_trick.cards c;
      (* More update if we finished the trick *)
      if n_players s == Dynarray.length s.current_trick.cards then (
        ...
        let cp_trick =
          { s.current_trick with cards = Dynarray.copy s.current_trick.cards }
        in
        Dynarray.add_last s.players.(winner).tricks cp_trick;
        prepare_next s.current_trick);
      (* Recursive call *)
...
let init distrib =
  ...
  let s =
    {
      players;
      current_trick = { idx = 0; cards = Dynarray.create () };
      first_player = 0;
    }
  in
...
let task_win_cards cards =
  let task player_index s =
    ...
    Dynarray.iter
      (fun trick ->
        Dynarray.iter
          (fun card -> Dynarray.add_last player_cards card)
          trick.cards)
      player.tricks;
